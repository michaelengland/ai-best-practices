# Workshop 1: Explorer → Strategist — Narrative

> **Next step:** Use **/deckwright:craft** to structure this narrative into a content outline.

## Overview

- **Audience**: People who have tried ChatGPT but use it like a search engine. They type vague prompts, get mediocre results, and blame the AI. Young, tech-savvy, but not engineers.
- **Purpose**: By the end, attendees can transform a vague prompt into a professional-quality one, understand that context matters more than phrasing, and have set up their first custom instruction (skill) in ChatGPT. They should feel the difference in real time — twice.
- **Key message**: Better words produce better results. Better information produces dramatically better results. Skills make it permanent.
- **Framework**: Show → Do → Compare (iterative rounds), then pivot to context engineering and skills
- **Tone**: Fast-paced lab. "Let's try it together" energy. No lectures — every concept is immediately practiced.
- **Format**: ~35 slides, ~55 min effective (1 hr session). Billboard design. **ChatGPT Org** — everyone already has this.

## Tool Requirements

This workshop uses **ChatGPT Org**, which all attendees have access to. No additional setup or accounts required.

- **Privacy**: Org conversations are not used to train ChatGPT — safe for company data

## Session Structure

### Opening: Where We're At (3 min)

- Show the spectrum. Highlight the full Explorer → Strategist jump. "Two levels in one session."
- "You've tried AI. Today you learn to talk to it properly — and then make it remember, so you never have to talk that well again."
- Quick level set: "AI apps respond based on what you put in. Today we change both the words AND the information."

### Exercise Setup (2 min)

- "Open ChatGPT on your laptop. We're going to transform the same task — first the words, then the information."
- Prescribe the task OR let them choose: "Pick a real task you do at work — a proposal, an email, a summary, a plan."
- **Prescribed fallback**: "Write me a proposal for improving our onboarding process."

---

## Act 1: The Words Matter (~20 min)

### Round 1: The Baseline (3 min)

- **Teach** (30 sec): "Most people type something vague and hope for the best. Let's see what that gets us."
- **Exercise**: Type your vague, natural prompt into ChatGPT. Read the output.
- **Discuss**: "Sound familiar? Generic. Surface-level. Could be for any company."

### Round 2: Assign a Role (3 min)

- **Teach** (30 sec): "Adding a role changes the AI's tone, depth, and perspective. One sentence of setup."
- **Exercise**: Add "You are a senior [relevant role]" to the beginning. Re-run.
- **Compare**: "Notice the terminology? The frameworks? The level of detail?"

### Round 3: Be Specific (3 min)

- **Teach** (30 sec): "Specificity in = specificity out. Tell it WHO it's for, WHAT the constraints are, HOW long it should be."
- **Exercise**: Add audience, constraints, and scope to the prompt. Re-run.
- **Compare**: The output now addresses a specific situation, not a generic one.

### Round 4: One Task at a Time (3 min)

- **Teach** (30 sec): "The kitchen-sink prompt — 5 requests tangled together — tries everything, nails nothing. Give it structure."
- **Exercise**: Structure the prompt with numbered sections or clear steps. Re-run.
- **Compare**: The AI mirrors your organisation.

### Round 5: Power Keywords (3 min)

- **Teach** (30 sec): "'Be radically honest.' 'Think step by step.' 'Challenge my assumptions.' These keywords change the AI's behaviour more than you'd expect."
- **Exercise**: Add 2–3 power keywords. Re-run.
- **Compare**: Notice the AI pushing back, flagging uncertainty, being more thoughtful.

### Round 6: Define the Output (3 min)

- **Teach** (30 sec): "You wouldn't ask a designer for 'something nice.' Tell the AI what format, tone, length, and audience you need."
- **Exercise**: Add output definition. Re-run.
- **Compare**: The output is now immediately usable — right format, right tone, right length.

### The First Reveal (2 min)

- "Open your first output and your last output side by side."
- "Same task. Same information. Just better words."
- This is the first emotional peak — the transformation is visceral because they did it themselves.

---

## Act 2: The Information Matters More (~15 min)

### The Pivot (1 min)

- "Five techniques. Dramatic improvement. But we never changed what the AI knew."
- "Watch what happens when we do."

### Exercise 7: Add Context (4 min)

- **Teach** (1 min): "Every round so far changed the words. Now we change the information."
- **Exercise**: Take the prompt from Round 6. Add 3–4 sentences of real context: "Our company has 500 employees. Current onboarding takes 4 weeks. HR team of 3. Last attempt to fix this failed because..."
- **Compare**: Output goes from "plausible but generic" to "sounds like someone who works here."
- **The aha**: "Same prompt. Different information. Dramatically bigger jump than all five techniques combined."

### Meet Your AI Colleague (1 min)

- Show the blue tang (Dory) image.
- "Brilliant. Helpful. Remembers nothing."
- "Every conversation starts from scratch. She doesn't know your company, your role, or what you told her yesterday."

### Exercise 8: Let AI Tell You What It Needs (4 min)

- **Teach** (30 sec): "Instead of guessing what context to provide, let AI ask."
- **Exercise**: Add to the prompt: "Before you start, ask me clarifying questions about anything you'd need to know."
- **Discuss**: "Look at what it asked. Did it surface things you hadn't thought of? Budget constraints? Stakeholder concerns? Timeline?"
- This is the single highest-leverage technique.

### The Problem: From Scratch Every Time (1 min)

- "You just pasted all that context manually. Your role, your company, your preferences."
- "Next conversation? You'll do it all again."
- "What if the AI already knew?"

---

## Act 3: Make It Stick (~12 min)

### Exercise 9: Set Up Your First Skill (8 min)

- **Teach** (2 min): Walk through ChatGPT's Custom Instructions settings. Screen-share the location: Settings → Personalisation → Custom Instructions. Wait for everyone to find it.
- **Exercise** (4 min): Set up a custom instruction:
  - "I work at [company]. My role is [role]."
  - "When writing proposals, use [format]. Tone: [style]. Audience: [who]."
  - "Be radically honest. Challenge my assumptions. Flag uncertainty."
- **Test it** (2 min): Open a NEW conversation. Type ONLY the task — no role, no context, no format. Just: "Write a proposal for reducing onboarding from 4 to 2 weeks."
- **The aha**: "You typed one sentence. It already knew the rest."

### The Grand Reveal (2 min)

- Three outputs side by side: Vague → Well-prompted → Skill-enhanced.
- "Same task. Three versions. Each one dramatically better."
- This is the second emotional peak — and it's bigger than the first.

### The GPS Metaphor (1 min)

- "Prompt engineering = giving directions every trip. Skills = a GPS that remembers."
- "One requires effort every time. The other compounds permanently."
- "Custom instructions, system prompts, project rules — all the same thing. All skills."

---

### Closing: What's Next (3 min)

- "You're now a Strategist. You don't just talk to AI well — you've set up its playbook."
- **The cliffhanger**: "But who's doing all the work? Still you. You search, you paste, you copy, you apply. Every step has 'you' in it."
- "Next time: Strategist → Orchestrator. We hand the AI the keys — and build it a team."

## Notes

- **Pacing for tech-savvy audiences**: The six prompt rounds should move fast — 30 seconds of teaching, 2 minutes of doing. Don't linger. The cumulative effect matters more than any single round. If the group is flying, tighten the rounds; if they're exploring, let them breathe.
- **Running example consistency**: Attendees should save their final prompt and custom instructions. These carry forward to Workshop 2.
- **Exercise 9 is the linchpin**: If setting up custom instructions stalls (people struggling with settings), the session stalls. Prepare a screen-share walkthrough. Verify the ChatGPT settings location before the session — it changes periodically.
- **Prescribed vs. free choice**: Free choice is more engaging but harder to support. Recommend offering both: "Use your own task, or follow along with our example."
- **The two emotional peaks**: The first reveal (Round 6 vs baseline) shows the power of words. The grand reveal (skill-enhanced vs baseline) shows the power of information. The second should feel bigger — if it doesn't, the context wasn't specific enough. Coach the room during Exercise 7.
- **If time is tight**: The grand reveal (three-way comparison) can become a facilitator demo instead of individual exercise. Don't cut the custom instructions setup — that's the takeaway they'll use tomorrow.
